...
module Unix : sig
  ...
  val server_of_fd : ?trace:tracer -> Tls.Config.server -> Uwt.Tcp.t -> t Lwt.t

  (** [client_of_fd ?tracer client ~host fd] is [t], after client-side
      TLS handshake of [fd] using [client] configuration and [host]. *)
  val client_of_fd : ?trace:tracer -> Tls.Config.client -> ?host:string -> Uwt.Tcp.t -> t Lwt.t

  (** [accept ?tracer server fd] is [t, sockaddr], after accepting a
      client on [fd] and upgrading to a TLS connection. *)
  val accept  : ?trace:tracer -> Tls.Config.server -> Uwt.Tcp.t -> (t * Uwt.sockaddr) Lwt.t
...
val accept_ext :
  ?trace:tracer -> Tls.Config.server -> Uwt.Tcp.t ->
  ((ic * oc) * Uwt.sockaddr) Lwt.t
...
val accept :
  ?trace:tracer -> Tls.Config.own_cert -> Uwt.Tcp.t ->
  ((ic * oc) * Uwt.sockaddr) Lwt.t
...
val connect :
  ?trace:tracer -> X509_uwt.authenticator -> string * int -> (ic * oc) Lwt.t
